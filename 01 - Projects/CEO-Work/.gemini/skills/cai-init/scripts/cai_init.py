...
#!/usr/bin/env python3
"""
CAI Init — Project configuration bootstrap and config loader.

Config files (flat YAML per module):
  - .agents/cai/core/config.yaml (core settings — user_name, language, output_folder, etc.)
  - .agents/cai/{module}/config.yaml (module settings + core values merged in)

Usage:
  # Fast path — load all vars for a module (includes core vars)
  python cai_init.py load --module bmb --all --project-root /path

  # Load specific vars with optional defaults
  python cai_init.py load --module bmb --vars var1:default1,var2 --project-root /path

  # Load core only
  python cai_init.py load --all --project-root /path

  # Check if init is needed
  python cai_init.py check --project-root /path
  python cai_init.py check --module bmb --skill-path /path/to/skill --project-root /path

  # Resolve module defaults given core answers
  python cai_init.py resolve-defaults --module bmb --core-answers '{"output_folder":"..."}' --project-root /path

  # Write config from answered questions
  python cai_init.py write --answers '{"core": {...}, "bmb": {...}}' --project-root /path
"""
...
def find_project_root(llm_provided=None):
    """
    Find project root by looking for .agents/cai folder.
    ...
    if llm_provided:
        ...
        if (candidate / '.agents/cai').exists():
            return candidate
        # First run — .agents/cai won't exist yet but LLM path is still valid
        if candidate.is_dir():
            return candidate
    ...
    for start_dir in [Path.cwd(), Path(__file__).resolve().parent]:
        ...
        while current_dir != current_dir.parent:
            if (current_dir / '.agents/cai').exists():
                return current_dir
...
def find_target_module_yaml(module_code, project_root, skill_path=None):
    ...
    Search order:
      ...
      3. .agents/cai/{module_code}/module.yaml (installed module location)
    """
    search_paths = []
    ...
    if project_root and module_code:
        search_paths.append(Path(project_root) / '.agents/cai' / module_code / 'module.yaml')
...
def load_module_config(module_code, project_root):
    """Load config for a specific module from .agents/cai/{module}/config.yaml."""
    config_path = Path(project_root) / '.agents/cai' / module_code / 'config.yaml'
...
def apply_result_template(var_def, raw_value, context):
    ...
    E.g., result: "{project-root}/{value}" with value="_cai-output"
    becomes "/Users/foo/project/_cai-output"
    """
...
def cmd_load(args):
    ...
    if not project_root:
        print(json.dumps({'error': 'Project root not found (.agents/cai folder not detected)'}),
              file=sys.stderr)
        sys.exit(1)
...
def cmd_write(args):
    ...
    if core_answers_raw and core_def:
        ...
        # Write core config
        core_dir = project_root / '.agents/cai' / 'core'
    ...
    for module_code, module_answers_raw in answers.items():
        ...
        # Write module config
        module_dir = project_root / '.agents/cai' / module_code
...
def _write_config_file(path, data, module_label):
    ...
    with open(path, 'w', encoding='utf-8') as f:
        ...
        f.write(f'# Generated by cai-init\n')
...
def main():
    parser = argparse.ArgumentParser(
        description='CAI Init — Project configuration bootstrap and config loader.'
    )
