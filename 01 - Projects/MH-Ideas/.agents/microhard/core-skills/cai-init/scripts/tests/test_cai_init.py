...
class TestFindProjectRoot(unittest.TestCase):
    ...
    def test_finds_cai_folder(self):
        temp_dir = tempfile.mkdtemp()
        try:
            (Path(temp_dir) / '_cai').mkdir()
    ...
    def test_llm_provided_with_cai(self):
        temp_dir = tempfile.mkdtemp()
        try:
            (Path(temp_dir) / '_cai').mkdir()
    ...
    def test_llm_provided_without_cai_still_returns_dir(self):
        """First-run case: LLM provides path but _cai doesn't exist yet."""
        temp_dir = tempfile.mkdtemp()
        try:
            result = find_project_root(llm_provided=temp_dir)
            self.assertEqual(result.resolve(), Path(temp_dir).resolve())
...
class TestExpandTemplate(unittest.TestCase):
    ...
    def test_multiple_placeholders(self):
        result = expand_template(
            ...
            {'output_folder': '_cai-output', 'project-root': '/test'}
        )
        self.assertEqual(result, '_cai-output/planning')
...
class TestApplyResultTemplate(unittest.TestCase):
    ...
    def test_with_result_template(self):
        ...
        result = apply_result_template(var_def, '_cai-output', {'project-root': '/test'})
        self.assertEqual(result, '/test/_cai-output')
...
class TestLoadModuleYaml(unittest.TestCase):
    ...
    def test_loads_core_module_yaml(self):
        ...
        self.assertEqual(result['meta']['name'], 'CAI Core Module')
...
class TestFindTargetModuleYaml(unittest.TestCase):
    ...
    def test_finds_in_cai_module_dir(self):
        module_dir = self.project_root / '_cai' / 'mymod'
    ...
    def test_skill_path_takes_priority(self):
        """Skill assets module.yaml takes priority over .agents/cai/{module}/."""
        skill_path = self.project_root / 'skills' / 'test-skill'
        assets = skill_path / 'assets'
        assets.mkdir(parents=True)
        (assets / 'module.yaml').write_text('code: test\nname: from-skill\n')

        module_dir = self.project_root / '_cai' / 'test'
        module_dir.mkdir(parents=True)
...
class TestLoadModuleConfig(unittest.TestCase):
    ...
    def setUp(self):
        self.temp_dir = tempfile.mkdtemp()
        self.project_root = Path(self.temp_dir)
        cai_core = self.project_root / '_cai' / 'core'
        cai_core.mkdir(parents=True)
        (cai_core / 'config.yaml').write_text(
            'user_name: TestUser\n'
            'communication_language: English\n'
            'document_output_language: English\n'
            'output_folder: "{project-root}/_cai-output"\n'
        )
        cai_bmb = self.project_root / '_cai' / 'bmb'
        cai_bmb.mkdir(parents=True)
        (cai_bmb / 'config.yaml').write_text(
            'user_name: TestUser\n'
            'communication_language: English\n'
            'document_output_language: English\n'
            'output_folder: "{project-root}/_cai-output"\n'
            'cai_builder_output_folder: "{project-root}/_cai-output/skills"\n'
            'cai_builder_reports: "{project-root}/_cai-output/reports"\n'
        )
    ...
    def test_load_module_includes_core_vars(self):
        ...
        self.assertIsNotNone(result)
        # Module-specific var
        self.assertIn('cai_builder_output_folder', result)
