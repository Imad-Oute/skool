...
platforms with no shared methodology layer. CAI is uniquely positioned to be the
...
building on the open-source skills CLI ecosystem, CAI offloads the highest-churn
...
```
## Differentiation
- Anti-fragmentation positioning: CAI = cross-platform constant across 40+ fragmenting AI tools; no competitor provides shared methodology layer
- Platform complexity delegated to Vercel skills CLI ecosystem (MIT); CAI maintains methodology, not platform configs
```
...
- **Key gap**: NO competitor combines structured methodology with plugin
  marketplace — this is CAI's whitespace
```
...
```
cai-init must always be included as a base skill in every bundle
```

**Discovery notes say:**

```
cai-init must always be included as a base skill in every bundle/install
...
```
- cai-init: always included as base skill in every bundle (solves bootstrapping)
```
...
type: cai-distillate
sources:
  - 'product-brief-cai-next-gen-installer.md'
  - 'product-brief-cai-next-gen-installer-discovery-notes.md'
downstream_consumer: 'PRD creation'
created: '2026-03-13'
token_estimate: 1450
parts: 1
...
- CAI Next-Gen Installer: replaces monolithic Node.js CLI with skill-based plugin architecture for distributing CAI methodology across 40+ AI platforms
- Three layers: self-describing plugins (cai-manifest.json), cross-platform install via Vercel skills CLI (MIT), runtime registration via cai-init skill
- Transforms CAI from dev-only methodology into open platform for any domain (creative, therapeutic, educational, personal)
...
- Plugins: skill bundles with Anthropic plugin standard as base format + cai-manifest.json extending for CAI-specific metadata (installer options, capabilities, help integration, phase ordering, dependencies)
- Existing manifest example: `{"module-code":"bmm","replaces-skill":"cai-create-product-brief","capabilities":[{"name":"create-brief","menu-code":"CB","supports-headless":true,"phase-name":"1-analysis","after":["brainstorming"],"before":["create-prd"],"is-required":true}]}`
- Vercel skills CLI handles platform translation; integration pattern (wrap/fork/call) is PRD decision
- cai-init: global skill scanning installed cai-manifest.json files, registering capabilities, configuring project settings; always included as base skill in every bundle (solves bootstrapping)
- cai-update: plugin update path without full reinstall; technical approach (diff/replace/preserve customizations) is PRD decision
...
- Anti-fragmentation: CAI = cross-platform constant; no competitor provides shared methodology layer across AI tools
- Curated quality: all submissions gated, human-reviewed by CAI + core team; 13.4% of community skills have critical vulnerabilities (Snyk 2026); quality gate value increases as ecosystem gets noisier
- Domain-agnostic: no competitor builds beyond software dev workflows; same plugin system powers any domain via CAI Builder (separate initiative)
...
- Future plugin creators: non-dev authors using CAI Builder; need distribution without building own installer
...
- cai-init discovers/registers all plugins from manifests; clear errors for malformed manifests
- At least one external module author successfully publishes plugin using manifest system
- cai-update works without full reinstall
- Existing CLI users have documented migration path

## Scope

- In: manifest spec, cai-init, cai-update, Vercel CLI integration, NPX installer, zip bundles, migration path
- Out: CAI Builder, marketplace web platform, skill conversion (prerequisite, separate), one-click install for all platforms, monetization, quality certification process (gated-submission principle is architectural requirement; process defined separately)
...
- Entry: `tools/cli/cai-cli.js` (Commander.js) → `tools/cli/installers/lib/core/installer.js`
...
- Skills already use directory-per-skill layout; cai-manifest.json sidecars exist but are not source of truth
...
- cai-update mechanics: diff/replace? Preserve user customizations?
...
- cai-manifest.json as open standard submission to Agent Skills governance?
...
- Module authors as acquisition channel: each published plugin distributes CAI to creator's audience
- CI/CD integration: cai-init as pipeline one-liner increases stickiness
- Educational institutions: structured methodology + non-technical install → university AI curriculum
- Skill composability: mixing CAI modules with third-party skills for custom methodology stacks
