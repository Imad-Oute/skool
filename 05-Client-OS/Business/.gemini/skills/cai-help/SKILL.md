---
name: cai-help
description: 'Use when user asks for help, what to do next, or what to start with. Analyzes current state and recommends the next skill(s) to use across any installed MicroHard module.'
---
...
Help the user understand where they are in their MicroHard workflow and what to do next. Answer questions about installed modules when asked.
...
- **Catalog**: `{project-root}/.agents/microhard/_config/microhard-help.csv` — assembled manifest of all installed module skills
- **Config**: `config.yaml` and `user-config.yaml` files in `{project-root}/.agents/microhard/` and its subfolders — resolve `output-location` variables, provide `communication_language` and `project_knowledge`
...
- Skill name in backticks — e.g., `cai-create-prd`
